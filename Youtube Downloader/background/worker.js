...
/**
 * Lightweight version of startDownload — allocates a task ID and sets
 * initial state in storage without starting any polling.  The popup uses
 * this so it can run the full download pipeline itself (avoiding the MV3
 * service-worker lifetime limit for long async tasks).
 */
async function initDownloadTask(video, format) {
  ...
  const nextPreferences = { ...preferences, preferredFormat: format };
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    switch (message?.type) {
      ...
      case MESSAGE_TYPES.INIT_DOWNLOAD:
        sendResponse({ ok: true, data: await initDownloadTask(message.video, message.format) });
        return;

      case MESSAGE_TYPES.UPDATE_TASK_STATE:
        sendResponse({ ok: true, data: await updateTaskState(message.partial || {}) });
        return;
